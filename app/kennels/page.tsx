...
function buildKennelCardHTML(animal: Animal, kennel: string, medRecords: MedicalRecord[], popupNoteCount = 0): string {
  ...
  const alertsHtml = [
    ...
    animal.status === "Hold" && animal.hold_type ? `<div style="background:#eff6ff;border:2px solid #3b82f6;border-radius:5px;padding:5px 10px;font-size:11px;font-weight:700;color:#1d4ed8;margin-bottom:6px;print-color-adjust:exact;-webkit-print-color-adjust:exact;">🔒 ${animal.hold_type}${animal.hold_type === "Adoption Hold" && animal.hold_adopter_info ? ` — ${((animal.hold_adopter_info as Record<string,unknown>).first_name || "")} ${((animal.hold_adopter_info as Record<string,unknown>).last_name || "")}` : ""}${animal.hold_type === "Legal Hold" && animal.hold_legal_info ? ` — Case #${((animal.hold_legal_info as Record<string,unknown>).case_number || "")}` : ""}${animal.hold_type === "Stray Hold" && animal.hold_end_date ? ` — Expires ${animal.hold_end_date}` : ""}</div>` : "",
  ].filter(Boolean).join("");

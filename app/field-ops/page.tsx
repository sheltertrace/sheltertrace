...
function OfficerCard({ officer }: { officer: OfficerFieldProfile }) {
  const status  = (officer.current_field_status as FieldStatus) || "Off Duty";
  const colors  = STATUS_COLORS[status] ?? STATUS_COLORS["Off Duty"];
  const onDuty  = status !== "Off Duty";
  const hasGPS  = officer.last_location_lat != null && officer.last_location_lng != null;

  return (
    <div style={{ background: "#fff", border: "1px solid #e0e0e0", borderRadius: 10, padding: "14px 18px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
        <div style={{ fontWeight: 700, fontSize: 15 }}>
          {officer.first_name} {officer.last_name}
        </div>
        <span style={{ background: colors.bg, color: colors.text, borderRadius: 20, padding: "3px 12px", fontSize: 12, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>
      ...
      <div style={{ fontSize: 12, color: "#555", marginBottom: 4 }}>
        {officer.badge ? `Badge ${officer.badge}` : `@${officer.username}`}
        {officer.role !== "Officer" ? ` · ${officer.role}` : ""}
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 10, fontSize: 12, color: "#888" }}>
        <span>Updated: {elapsed(officer.last_status_update)}</span>
        {onDuty && (
          hasGPS ? (
        ...
        {officer.tracking_active && (
          <span style={{ color: "#16a34a", fontSize: 11, fontWeight: 700, background: "#dcfce7", padding: "1px 6px", borderRadius: 8 }}>📍 Tracking</span>
        )}
      </div>

      {officer.phone && (
        <a href={`tel:${officer.phone}`} style={{ fontSize: 12, color: "var(--teal)", textDecoration: "none", marginTop: 4, display: "block" }}>
          📱 {officer.phone}
        </a>
      )}
    </div>
...
export default function FieldOpsPage() {
  ...
  // Faster GPS-only refresh every 15 s
  useEffect(() => {
  ...
  // ── Derived data ──────────────────────────────────────────────────────────
  // Status comes directly from staff_accounts.current_field_status — no schedule lookup.
  const activeOfficers = officers.filter((o) => o.current_field_status !== "Off Duty");
  const activeCount    = activeOfficers.length;

  const groups = (["On Scene","En Route","On Duty","Available","Break","Off Duty"] as FieldStatus[])
    .map((s) => ({
      status: s,
      items:  officers.filter((o) => (o.current_field_status || "Off Duty") === s),
    }))
      ...
      <div style={{ padding: "24px", maxWidth: 1280, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", gap: 12, marginBottom: 28, flexWrap: "wrap" }}>
          {(["On Duty","En Route","On Scene","Available","Break","Off Duty"] as FieldStatus[]).map((s) => {
            const count  = officers.filter((o) => (o.current_field_status || "Off Duty") === s).length;
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <span style={{ fontSize: 11, color: "#94a3b8" }}>
                Updated {Math.round((Date.now() - lastRefresh.getTime()) / 1000)}s ago · refreshes every 15s
              </span>
              <span style={{ fontSize: 12, color: "#64748b" }}>{mapOpen ? "▼" : "▶"}</span>
            </div>
          </div>
          {mapOpen && <OfficerMap officers={officers} height={460} />}
        </div>
        ...
        <div style={{ marginBottom: 32 }}>
          ...
          {loading ? (
            ...
            <p style={{ color: "#888" }}>No active staff found. Add staff in Admin.</p>
          ) : (
              ...
              {groups.map((g) => (
                <div key={g.status}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(260px, 1fr))", gap: 10 }}>
                    {g.items.map((o) => <OfficerCard key={o.id} officer={o} />)}
                  </div>
                </div>
              ))}
            </div>

...
import type { StaffAccount, Person, DispatchCall, Animal } from "@/lib/types";
import {
...
export type IntakeChoice = "surrender" | "stray" | "field" | null;
...
interface Props {
  ...
  // Embedded (modal/drawer) mode — when provided, the wizard calls these
  // instead of navigating to /officer-app, so it never leaves the page it
  // was opened from (a dispatch call, for example).
  onClose?: () => void;
  onIntakeComplete?: (animal: Animal) => void;
}

export default function FieldIntakeWizard({ officer, prefillCallId, prefillAddress, onClose, onIntakeComplete }: Props) {
  ...
  useEffect(() => {
    ...
    }).catch(() => setLoadError("Could not load contacts — you can still search and add a new person manually."));
    ...
    if (draft) {
      ...
      setFoundAddress(draft.foundAddress || prefillAddress || ""); setFoundCity(draft.foundCity ?? "");
      ...
      setLinkedCallId(draft.linkedCallId || prefillCallId || "");
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      onIntakeComplete?.(result.animal);
    } catch (err) {
      ...
      <div style={{ minHeight: onClose ? undefined : "100dvh", background: "#0f2942", color: "#e2e8f0", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", borderRadius: onClose ? 14 : undefined, maxHeight: onClose ? "90vh" : undefined, overflowY: onClose ? "auto" : undefined }}>
        <Screen>
          <div style={{ textAlign: "center", paddingTop: 30 }}>
            ...
            {onClose ? (
              <button onClick={onClose} style={{ display: "block", width: "100%", marginTop: 24, padding: "16px 0", borderRadius: 12, background: "#16a34a", border: "none", color: "#fff", fontSize: 15, fontWeight: 800, cursor: "pointer" }}>
                ✓ Done
              </button>
            ) : (
  ...
  return (
    <div style={{ minHeight: onClose ? undefined : "100dvh", maxHeight: onClose ? "90vh" : undefined, overflowY: onClose ? "auto" : undefined, background: "#0f2942", color: "#e2e8f0", fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", borderRadius: onClose ? 14 : undefined }}>
      ...
      {loadError && (
        <div style={{ background: "#7c2d12", color: "#fed7aa", padding: "8px 16px", fontSize: 12, textAlign: "center", fontWeight: 700 }}>
          ⚠️ {loadError}
        </div>
      )}
      {queueCount > 0 && (
      ...
      {/* Sticky bottom nav */}
      {currentStepId !== "review" && (
        <div style={{ position: onClose ? "sticky" : "fixed", bottom: 0, left: 0, right: 0, background: "#071e33", borderTop: "1px solid #1a3a5c", padding: "12px 16px", display: "flex", gap: 10, zIndex: 50 }}>
      ...
      {currentStepId === "review" && stepIdx > 0 && (
        <div style={{ position: onClose ? "sticky" : "fixed", bottom: 0, left: 0, right: 0, background: "#071e33", borderTop: "1px solid #1a3a5c", padding: "12px 16px", zIndex: 50 }}>
